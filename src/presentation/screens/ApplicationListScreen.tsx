...
export const ApplicationListScreen: React.FC = () => {
    ...
    const ApplicationCard = React.memo(({ item }: { item: DisplayApplication }) => {
            ...
            >
                ...
                <View className="flex-1">
                    <View className="flex-row items-center justify-between mb-1">
                        ...
                        <View className="flex-row items-center px-2 py-0.5 rounded-full" style={{ backgroundColor: config.bg }}>
                            <View className="w-1.5 h-1.5 rounded-full mr-1.5" style={{ backgroundColor: config.dot }} />
                ...
                <View className="ml-3 bg-slate-50 p-2 rounded-xl">
                    <ChevronRight color={COLORS.muted} size={16} strokeWidth={3} />
                </View>
            </TouchableOpacity>
        );
    });

    const renderItem = ({ item }: { item: DisplayApplication }) => <ApplicationCard item={item} />;
                ...
                <View className="bg-white px-6 pt-6 pb-8 border-b border-slate-100 mb-6">
                    ...
                    <View className="flex-row justify-between items-center">
                        ...
                        <View className="w-10 h-10 bg-slate-50 rounded-full items-center justify-center border border-slate-100">
                            <Users color={COLORS.sub} size={20} />
                        </View>
                    </View>
                </View>
